...
class HomeViewController: UIViewController {
    ...
    @IBOutlet weak var speciesLibraryColView: UICollectionView!
     @IBOutlet weak var wildLifeColView: UICollectionView!
    
    
    @IBOutlet weak var librarySeeAll: UILabel!
    @IBOutlet weak var lawsSellAll: UILabel!
    ...
    override func viewDidLoad() {
        ...
        speciesLibraryColView.reloadData()
        
        let tap1 = UITapGestureRecognizer(target: self, action: #selector(libraryAllTap(sender:)))
        librarySeeAll.isUserInteractionEnabled = true
        librarySeeAll.addGestureRecognizer(tap1)
        
        let tap2 = UITapGestureRecognizer(target: self, action: #selector(lawsAllTap(sender:)))
        lawsSellAll.isUserInteractionEnabled = true
        lawsSellAll.addGestureRecognizer(tap2)
        
     
       
    }
    @objc func libraryAllTap(sender:UITapGestureRecognizer){
        let vc = self.storyboard?.instantiateViewController(withIdentifier: "SpeciesLibraryVC") as! SpeciesLibraryVC
        self.navigationController?.pushViewController(vc, animated: true)
         
    }
    @objc func lawsAllTap(sender:UITapGestureRecognizer){
        let vc = self.storyboard?.instantiateViewController(withIdentifier: "WildLifeVC") as! WildLifeVC
        self.navigationController?.pushViewController(vc, animated: true)
    }
...
extension HomeViewController : UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout{
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        if collectionView == speciesLibraryColView{
            return specieImages.count
        }else{
            
            return lawImages.count
        }
    }

    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if collectionView == speciesLibraryColView{
            let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "SourceLibraryColCell", for: indexPath) as! SourceLibraryColCell
                guard indexPath.row < specieImages.count else {return cell}
                let instance =  specieImages[indexPath.row]
            cell.itemImage.image = UIImage(named: instance)
//            cell.deleteBtn.tag = indexPath.row
//            cell.deleteBtn.addTarget(self, action: #selector(crossBtnTapped(sender:)), for: .touchUpInside)

            return cell
        }else{
            let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "WildLifeLawsColCell", for: indexPath) as! WildLifeLawsColCell
                guard indexPath.row < lawImages.count else {return cell}
                let instance =  lawImages[indexPath.row]
            cell.itemImage.image = UIImage(named: instance)
//            cell.deleteBtn.tag = indexPath.row
//            cell.deleteBtn.addTarget(self, action: #selector(crossBtnTapped(sender:)), for: .touchUpInside)
